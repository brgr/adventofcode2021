...
impl MyRange {
    fn new(from: i32, to: i32) -> Self { MyRange { from, to } }
...
#[test]
fn test_overlaps() {
    assert!(MyRange::new(10, 15).overlaps(&MyRange::new(15, 20)));
    assert!(!MyRange::new(10, 15).overlaps(&MyRange::new(17, 20)));
    assert!(MyRange::new(-10, 15).overlaps(&MyRange::new(-15, -7)));
    assert!(!MyRange::new(-10, 15).overlaps(&MyRange::new(-15, -11)));
    assert!(MyRange::new(2, 3).overlaps(&MyRange::new(-5, 20)));
}
...
impl FromStr for RebootStep {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let (turn_on_str, ranges_str) = s.split_once(" ").unwrap();

        let turn_on = match turn_on_str {
            "on" => true,
            "off" => false,
            _ => panic!()
        };

        let (x_range_str, y_range_str, z_range_str): (&str, &str, &str) =
            ranges_str.split(",")
                .map(|r| r.split_once("=").unwrap().1)
                .collect_tuple().unwrap();

        let (x_range_from, x_range_to) = x_range_str.split_once("..").unwrap();
        let (y_range_from, y_range_to) = y_range_str.split_once("..").unwrap();
        let (z_range_from, z_range_to) = z_range_str.split_once("..").unwrap();

        Ok(RebootStep {
            turn_on,
            x_range: MyRange::new(x_range_from.parse().unwrap(), x_range_to.parse().unwrap()),
            y_range: MyRange::new(y_range_from.parse().unwrap(), y_range_to.parse().unwrap()),
            z_range: MyRange::new(z_range_from.parse().unwrap(), z_range_to.parse().unwrap()),
        })
    }
}
...
fn part1() {
    ...
    //  I think I *do* need to handle polygons, one way or another.
    //  I think the easiest way to do so would be to split a cube into multiple cubes on
    //  intersection...
    //  ------------------------
    /// After thinking some more about it, I have found another way to do it:
    /// Still save for every point whether it's turned on or not. However, saving it just like
    /// that will definitely be to much. For instance, our input goes from roughly -100_000 up to
    /// 100_000. I.e. we have a range of 200_000, which gives us a total point count of
    /// 200_000 ^ 3 = 8 * 10^15. This is in bits. This is around 1_000 TB! Way too much
    /// to store completely.
    /// But there is another way to store it: Sparse Matrices!
    /// Since we have lots and lots of areas where there are just 1s or 0s (turned on/off), we can
    /// combine these using ranges: We say that we have 0s from 0 to 12345, then 1s from 12346 to...
    /// The remaining problem is on how we can do this with 3 dimensions. The easiest way is
    /// probably to make it 1-dimensional. For that we define an upper bound, for instance
    /// upper bound *u* = 200_000. (For simplicity, let's start with (0,0,0) and only count
    /// upwards, then when inserting, increase by u/2, and when reading in the end, decrease
    /// by u/2)
    /// Our elements in our sparse matrix will now actually be elements in a sparse vector.
    /// This vector has "size" of u^3. Size is in quotation marks because it won't be the actual
    /// size of this vector, but instead the maximum size of any end of any range in the vector.
    /// To calculate the position of any (x,y,z) in the vector, use this formula:
    /// vector_position = x + u * y + u^2 * z
    /// A simple test gives us: (0,0,0) is at pos 0, (u,u,u) is at pos
    /// u + u^2 + u^3
    /// TODO: This is somehow larger than u^3!? But I still think it's correct...
    /// Since in our sparse vector the x elements will be next to each other, this means
    /// that we can now save ranges of x for every (y,z) pair, when adding a cuboid, and removing
    /// or splitting those, when removing / turning off a cuboid.


    let reboot_steps: Vec<RebootStep> = E2.lines().map(|l| RebootStep::from_str(l).unwrap())
        .filter(|step|
            step.x_range.from >= -50 && step.x_range.to <= 50 &&
                step.y_range.from >= -50 && step.y_range.to <= 50 &&
                step.z_range.from >= -50 && step.z_range.to <= 50)
        .collect();

    println!("{:?}", reboot_steps);

    let mut x_ranges: Vec<MyRange> = Vec::new();
    let mut y_ranges: Vec<MyRange> = Vec::new();
    let mut z_ranges: Vec<MyRange> = Vec::new();

    for reboot_step in reboot_steps {
        if reboot_step.turn_on {
            x_ranges = insert_range(&x_ranges, reboot_step.x_range);
            y_ranges = insert_range(&y_ranges, reboot_step.y_range);
            z_ranges = insert_range(&z_ranges, reboot_step.z_range);
        } else {
            x_ranges = remove_range(&x_ranges, reboot_step.x_range);
            y_ranges = remove_range(&y_ranges, reboot_step.y_range);
            z_ranges = remove_range(&z_ranges, reboot_step.z_range);
        }
        println!("z ranges: {:?}", z_ranges);
    }

    println!("end x ranges: {:?}", x_ranges);
    println!("end y ranges: {:?}", y_ranges);
    println!("end z ranges: {:?}", z_ranges);

    let x_total_size = total_size(&x_ranges);
    let y_total_size = total_size(&y_ranges);
    let z_total_size = total_size(&z_ranges);

    println!("total sizes (x,y,z): {:?}", (x_total_size, y_total_size, z_total_size));

    let total_count = (x_total_size * y_total_size * z_total_size) + (y_total_size * z_total_size) + z_total_size;

    println!("{}", total_count);

}

fn total_size(ranges: &Vec<MyRange>) -> u64 {
    let mut total_size = 0;

    for range in ranges {
        total_size += (range.to - range.from + 1) as u64;
    }

    total_size
}

fn remove_range(ranges: &Vec<MyRange>, range_to_remove: MyRange) -> Vec<MyRange> {
    ...
    for original_range in ranges {
        ...
        if original_range.overlaps(&range_to_remove) {
            if original_range.from >= range_to_remove.from && original_range.to <= range_to_remove.to {
                // If the whole range is overlapped, remove it completely (don't add it)
                continue;
            } else if original_range.from < range_to_remove.from && original_range.to > range_to_remove.to {
                // If range_to_remove is part of original_range, split original_range in two
                new_ranges.push(MyRange::new(original_range.from, range_to_remove.from - 1));
                new_ranges.push(MyRange::new(range_to_remove.to + 1, original_range.to));
            } else {
                let from = if original_range.from < range_to_remove.from { original_range.from } else { range_to_remove.to + 1 };
                let to = if original_range.to > range_to_remove.to { original_range.to } else { range_to_remove.from - 1 };
                new_ranges.push(MyRange::new(from, to));
            }
        } else {
...
fn insert_range(ranges: &Vec<MyRange>, new_range: MyRange) -> Vec<MyRange> {
    let mut new_ranges = Vec::new();
    ...
    let mut currently_overlapping = None;

    for original_range in ranges {
        if !original_range.overlaps(&new_range) {
            if let Some(range_from_before) = currently_overlapping {
                new_ranges.push(range_from_before);
                currently_overlapping = None;
            }
            new_ranges.push(*original_range);
        } else {
            let from = if original_range.from < new_range.from { original_range.from } else { new_range.from };
            let to = if original_range.to > new_range.to { original_range.to } else { new_range.to };
            if let Some(range_from_before) = currently_overlapping {
                let from = if from < range_from_before.from { from } else { range_from_before.from };
                let to = if to > range_from_before.to { to } else { range_from_before.to };
                let new_overlapping = MyRange::new(from, to);
                currently_overlapping = Some(new_overlapping);
            } else {
                currently_overlapping = Some(MyRange::new(from, to));
            }
        }
    }

    if let Some(overlapping_from_before) = currently_overlapping {
        new_ranges.push(overlapping_from_before);
    }
...
#[test]
fn test_insert() {
    let vec1 = insert_range(&vec![MyRange::new(-49, -5)], MyRange::new(-49, 47));
    println!("{:?}", vec1);

    let vec2 = insert_range(&vec![MyRange::new(-49, 47)], MyRange::new(-49, -5));
    println!("{:?}", vec2);

    let vec3 = insert_range(&vec![MyRange::new(-49, -49), MyRange::new(-21, 47)], MyRange::new(-49, 47));
    println!("{:?}", vec3);
}
